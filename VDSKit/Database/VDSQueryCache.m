    ...
    if (queryReference == nil) {
        if (error != NULL) {
            *error = [NSError errorWithDomain:VDSKitErrorDomain
                                     ...
                                     userInfo:@{NSDebugDescriptionErrorKey:VDS_NIL_ARGUMENT_MESSAGE(@"queryReference", _cmd)}];
        }
    } else {
                ...
                error:(NSError* _Nullable __autoreleasing * _Nullable)error
{
    ...
    if (queryReference == nil) {
        ...
        if (error != NULL) {
            *error = [NSError errorWithDomain:VDSKitErrorDomain
                                         code:VDSUnexpectedNilArgument
                                     userInfo:@{NSDebugDescriptionErrorKey:VDS_NIL_ARGUMENT_MESSAGE(@"entry", _cmd)}];
        }
    } else {
